...
<p>然而，在某些合理的设计场景中，这条规则会成为阻碍。<strong class="strong-star">内部可变性</strong> (Interior Mutability) 是一种设计模式，它允许你即使在持有不可变引用时，也能修改数据内部的值。</p>
<p>这听起来像是在绕开 Rust 的安全保障，实际上并非如此。<code>RefCell<T></code> 并没有绕过借用规则，它只是将借用检查从<strong class="strong-star">编译时</strong>推迟到了<strong class="strong-star">运行时</strong>。如果运行时违反了规则，程序会 Panic 而不是产生未定义行为。</p>
...
<table><thead><tr><th>类型</th><th>所有者数量</th><th>借用检查时机</th><th>可变性</th></tr></thead><tbody><tr><td><code>Box<T></code></td><td>唯一</td><td>编译时</td><td>可变或不可变</td></tr><tr><td><code>Rc<T></code></td><td>多个</td><td>编译时</td><td>仅不可变</td></tr><tr><td><code>RefCell<T></code></td><td>唯一</td><td><strong class="strong-star">运行时</strong></td><td>可变或不可变</td></tr></tbody></table>
<p><code>RefCell<T></code> 提供了两个核心方法：</p>
<ul>
<li><strong class="strong-star"><code>borrow()</code></strong>：返回 <code>Ref<T></code>，行为类似不可变引用 <code>&T</code>。</li>
<li><strong class="strong-star"><code>borrow_mut()</code></strong>：返回 <code>RefMut<T></code>，行为类似可变引用 <code>&mut T</code>。</li>
</ul>
...
<pre class="astro-code github-dark" style="background-color:#24292e;color:#e1e4e8; overflow-x: auto;" tabindex="0" data-language="text"><code><span class="line"><span>thread 'main' panicked at 'already borrowed: BorrowMutError'</span></span></code></pre>
<h3 id="何时选择-refcellt">何时选择 <code>RefCell<T></code></h3>
<p>当你<strong class="strong-star">确信</strong>代码在运行时不会违反借用规则，但编译器因为其分析的保守性而无法证明这一点时，<code>RefCell<T></code> 是正确的选择。</p>
...
<span class="line"><span style="color:#6A737D">    // &self 而非 &mut self</span></span>
...
<ul>
<li><code>Rc<T></code> 解决了<strong class="strong-star">多所有者</strong>的问题</li>
<li><code>RefCell<T></code> 解决了<strong class="strong-star">可变性</strong>的问题</li>
</ul>
