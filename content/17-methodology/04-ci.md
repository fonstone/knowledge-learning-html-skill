...
入门 ⏱ 20 分钟 持续集成GitHub ActionsCIcargo audit依赖管理安全审计
...
触发时机：推送到 main 或提 PR 时触发

`runs-on`

运行环境，`ubuntu-latest` 是最常用的
...
需要额外安装的组件（rustfmt、clippy 默认不包含）
...
docs.rs 上的文档是否完整、示例是否清晰

**依赖树**

`cargo tree` 查看会引入多少间接依赖

**许可证**

MIT/Apache-2.0 最宽松；GPL 有传染性，商业项目需谨慎

**维护者**

个人项目 vs 组织/公司维护，后者通常更稳定
...
允许 `1.2.3` 到 `<2.0.0`（推荐，兼容更新）

`"~1.2.3"`

允许 `1.2.3` 到 `<1.3.0`（只接受 patch 更新）
