...
入门 ⏱ 20 分钟 性能分析基准测试criterionflamegraphperfprofiling
...
预分配 `Vec::with_capacity`；使用 arena 分配器
...
用 `write!` 到 buffer；或 `join`

**HashMap 哈希函数慢**

大量 HashMap 操作占用时间

换用 `FxHashMap` / `AHashMap` 等更快的哈希实现

**迭代器中的条件分支**

循环内有大量 if/match

尝试提取不变条件到循环外；SIMD 优化

**同步 IO 阻塞**

线程长时间等待 IO

换用异步 IO（tokio/async-std）
