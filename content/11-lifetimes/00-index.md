<p>生命周期是 Rust 最独特的特性之一——它让编译器能够在<strong class="strong-star">不需要垃圾回收器</strong>的情况下，保证所有引用永远不会成为悬垂指针。</p>
