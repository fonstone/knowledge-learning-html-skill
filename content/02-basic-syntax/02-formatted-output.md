...
入门 ⏱ 20 分钟 println!format!格式化{:?}Debug#\[derive(Debug)\]格式规范
...
实现自定义类型的 `Display`（控制 `{}` 的输出格式）属于进阶内容，会在[格式化输出进阶](/RustCourse/chapters/22-advanced/06-advanced-formatting)中详细讲解。
