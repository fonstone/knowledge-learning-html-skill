...
1. build.rs 执行代码，从 git 读取 commit hash
1. build.rs 打印 cargo::rustc-env=GIT_HASH=abc123
1. Cargo 读取这一行输出 ，理解这是一条指令
1. Cargo 把 GIT_HASH=abc123 设置为环境变量
1. Cargo 调用 rustc 编译 src/main.rs
1. 编译时， env!("GIT_HASH") 展开为 "abc123"
1. 最终二进制中包含了编译时的 git 信息
