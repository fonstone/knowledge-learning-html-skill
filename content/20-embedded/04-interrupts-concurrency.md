...
高级 ⏱ 25 分钟 中断ISRMutexCriticalSection并发控制
...
**为什么需要 `cs` 令牌？** Rust 的嵌入式 `Mutex` 要求在调用 `borrow` 时必须传入一个 `CriticalSection` 令牌。由于获取令牌的唯一途径是调用 `cs::with`（这会禁用中断），这就保证了 **只要你在持有数据，中断就一定发不生**。
