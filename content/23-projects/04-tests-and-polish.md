...
进阶 ⏱ 30 分钟 Display单元测试彩色输出crossterm测试驱动格式化
...
`Todo`、`Command`、`TodoStore`

所有权与借用

`&Todo`（引用）vs `Todo`（所有权），`mark_done` 返回引用

错误处理

`Result<_, String>`、`?`、`.map_err()`

迭代器

`.find()`、`.position()`、`.filter()`、`.count()`、`.max()`
...
`#[derive(Serialize, Deserialize)]` 自动生成 JSON 读写
