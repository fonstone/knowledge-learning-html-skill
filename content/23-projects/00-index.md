...
入门 ⏱ 5 分钟 综合项目CLI任务管理器todoserdeJSON命令行搜索
...
`Todo { id, title, completed }` 及其方法

枚举

`Command`（Add/List/Done/Remove/Search/Help）
...
`.filter()` 搜索关键词、`.find()` 查 ID、`.position()` 定位下标
